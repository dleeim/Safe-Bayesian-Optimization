...
def plot_obj_con_outputs(data):
    ...
    # Find mean and std
    obj_fun_mean = jnp.mean(obj_fun_outputs,axis=0)
    obj_fun_std = jnp.std(obj_fun_outputs,axis=0,ddof=1)

    con_fun_mean = jnp.mean(con_fun_outputs,axis=0)
    con_fun_std = jnp.std(con_fun_outputs,axis=0,ddof=2)
        
    # Find confidence interval
    degree_of_freedom = n_sample-1
    confidence_interval = 0.975
    t_value = t.ppf(confidence_interval,degree_of_freedom)

    obj_fun_upper = obj_fun_mean + t_value*obj_fun_std/jnp.sqrt(n_sample)
    obj_fun_lower = obj_fun_mean - t_value*obj_fun_std/jnp.sqrt(n_sample)

    con_fun_upper = con_fun_mean + t_value*con_fun_std/jnp.sqrt(n_sample)
    con_fun_lower = con_fun_mean - t_value*con_fun_std/jnp.sqrt(n_sample)
...
# Load Data
data_SafeOpt = jnp.load('data/data_SafeOpt_Benoit.npz', allow_pickle=True)
